...
test('Oracle adapter reads full LONG expressions, ordered keys and actual index expressions', async () => {
  const longPredicate = '"VALUE" >= 0' + ' /* preserved metadata */'.repeat(2000);
  ...
  const connection = {
    async execute(sql: string) {
      ...
      else if (sql.includes('FROM dba_constraints c')) rows = [{ OWNER: 'APP', CONSTRAINT_NAME: 'CK_VALUE', CONSTRAINT_TYPE: 'C', GENERATED: 'USER NAME',
        STATUS: 'ENABLED', VALIDATED: 'VALIDATED', DEFERRABLE: 'NOT DEFERRABLE', DEFERRED: 'IMMEDIATE', RELY: null,
        SEARCH_CONDITION: longPredicate, INDEX_OWNER: null, INDEX_NAME: null, R_OWNER: null, R_CONSTRAINT_NAME: null, DELETE_RULE: null, PARENT_TABLE_NAME: null }];
      else if (sql.includes('FROM dba_indexes')) rows = [{ OWNER: 'APP', INDEX_NAME: 'IX_VALUE', INDEX_TYPE: 'FUNCTION-BASED NORMAL', UNIQUENESS: 'NONUNIQUE', VISIBILITY: 'VISIBLE', STATUS: 'VALID', PARTITIONED: 'NO', COMPRESSION: 'DISABLED' }];
      else if (sql.includes('FROM dba_ind_expressions')) rows = [{ COLUMN_POSITION: 1, COLUMN_EXPRESSION: 'ABS("VALUE")' }];
      else if (sql.includes('FROM dba_ind_columns')) rows = [{ COLUMN_NAME: 'SYS_NC00002$', COLUMN_POSITION: 1, DESCEND: 'ASC' }];
      let fetched = false;
