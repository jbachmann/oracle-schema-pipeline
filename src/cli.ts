...
async function main(): Promise<void> {
  const { values, positionals } = parseArgs({ allowPositionals: true, options: {
    ...
    tnsnames: { type: 'string' }, 'tns-alias': { type: 'string' }, 'catalog-scope': { type: 'string' },
...
Validation errors use exit code 2; unsupported models never produce SQL.`);
  ...
  if (command === 'extract') {
    ...
    if (scope !== 'all' && scope !== 'dba') throw new Error('--catalog-scope must be all or dba.');
    const { default: oracle } = await import('oracledb');
    const resolvedConnection = await resolveConnectionOptions({ dsn: values.dsn, tnsnames: values.tnsnames,
      tnsAlias: values['tns-alias'] }, configDir => oracle.getNetworkServiceNames(configDir));
    ...
    const [{ OracleCatalog }, { extractSource }, { readPassword }] = await Promise.all([
      import('./catalog.js'), import('./extract.js'), import('./password.js'),
    ]);
    const connection = await oracle.getConnection({
      user: requireOption(values.user, 'user'), ...resolvedConnection, password: await readPassword(),
    });
    try {
      connection.callTimeout = 300_000;
      const source = await extractSource(new OracleCatalog(connection, scope), selection);
