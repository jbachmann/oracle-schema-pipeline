    ...
    [...table.constraints]
      ...
      .flatMap((item) => {
        ...
        const row = (
          ...
          indexOwner,
          indexName,
          yes(item.state.enabled),
          yes(item.state.validated),
          yes(item.state.deferrable),
          yes(item.state.initiallyDeferred),
          yes(item.state.rely),
        ];
        if (item.kind === 'primary-key' || item.kind === 'unique')
          return item.columns.map((column, index) =>
            row(
              ...
              null,
              null,
              item.backingIndex?.owner ?? null,
              item.backingIndex?.name ?? null,
            ),
          );
        if (item.kind === 'foreign-key')
          return item.columnPairs.map((pair, index) =>
            row(
              index + 1,
              pair.childColumn,
              item.parentTable.owner,
              item.parentTable.name,
              item.parentConstraint.name,
              pair.parentColumn,
              null,
              item.onDelete,
              null,
        ...
        return [
          row(1, item.column, null, null, null, null, null, null, null, null),
        ];
      }),
  );
  return [
    ...
    },
    {
      ...
      long: ['Comment', 'Unsupported Features'],
      rows: tables.map((t) => [
        t.reference.owner,
        t.reference.name,
        t.role,
        t.comment,
        t.sourcePhysical.tablespace,
        t.sourcePhysical.compression,
        [...t.unsupportedFeatures].sort(cmp).join('\n'),
        t.columns.length,
    ...
    },
    {
      ...
      long: ['Comment', 'Default Expression', 'Identity Options'],
      rows: tables.flatMap((t) =>
        [...t.columns]
          .sort((a, b) => a.position - b.position || cmp(a.name, b.name))
          .map((c) => [
            t.reference.owner,
            t.reference.name,
            t.role,
            ...
            c.dataType.owner,
            c.dataType.name,
            c.dataType.byteLength,
            c.dataType.characterLength,
            c.dataType.lengthSemantics,
            ...
            yes(c.virtual),
            yes(c.invisible),
            c.identity?.generation ?? null,
            c.identity?.options ?? null,
            c.collation,
          ]),
      ),
    },
    ...
    },
    {
      ...
      long: ['Columns', 'Query', 'Unsupported Features'],
      rows: views.map((v) => [
        v.reference.owner,
        v.reference.name,
        v.role,
        ...
        yes(v.superview),
        yes(v.containerData),
        [...v.unsupportedFeatures].sort(cmp).join('\n'),
      ]),
    },
    {
    ...
    },
    {
      ...
      rows: [...source.diagnostics]
        ...
        .map((d) => [d.severity, d.code, d.object, d.message]),
    },
  ];
}
...
function validate(definition: Definition): void {
  ...
  definition.rows.forEach((row, r) =>
    row.forEach((value, c) => {
      if (typeof value !== 'string') return;
      const object = String(row[0] ?? `row ${r + 2}`),
        field = definition.headers[c]!;
      ...
      if (lineFeeds > MAX_LFS)
        throw new Error(
          `Workbook line-feed limit exceeded: ${definition.name} ${object} ${field} has ${lineFeeds} line feeds; maximum ${MAX_LFS}.`,
        );
    }),
  );
}
...
export function buildDictionaryWorkbook(
  ...
  const workbook = new ExcelJS.Workbook(),
    timestamp = new Date(source.extractedAt);
  ...
  for (const definition of definitions(source)) {
    ...
    definition.headers.forEach((header, index) => {
      ...
      column.width = definition.long.includes(header)
        ? 48
        : Math.min(24, Math.max(12, header.length + 2));
      if (definition.long.includes(header))
