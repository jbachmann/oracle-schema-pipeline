...
export async function readPassword(): Promise<string> {
  ...
  if (!input.isTTY) throw new Error('Set ORACLE_PASSWORD when running without an interactive terminal');
  ...
  return new Promise((resolvePassword, reject) => {
    ...
    const onData = (data: string): void => {
      for (const character of data) {
        if (character === '\u0003' || character === '\u0004') { cleanup(); reject(new Error('Password entry cancelled')); return; }
